...
This log tracks all findings from red team reviews during
commits. Findings start in "Open" and move to "Resolved"
when fixed.
...
### RT-001 — Effort parser panics on multibyte chars
...
- **Description:** Rust's `f64::parse` accepts "inf",
  ...
  doesn't catch NaN (since `NaN > 0.0` is false... wait,
  `NaN <= 0.0` is also false) or positive infinity. These
  values would be stored in JSON and produce nonsensical
  output.
...
- **Category:** Code Quality
- **Commit context:** Coverage infrastructure
- **Description:** `cmd_batch` called `std::process::exit(1)`
  on batch failure, bypassing Rust's drop/flush guarantees.
  On Windows (where stdout is not line-buffered when piped),
  the error JSON output could be truncated or lost entirely.
